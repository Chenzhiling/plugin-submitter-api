package com.czl.submitter.flink.service.tool
 ...
 * Date: 2022/5/27
 * Description:  restful to submit flink task
 */
object FlinkSubmitUtils {
  ...
  def queryTaskViaRestApi(url: String): QueryResponse = {
    val response: String = Request.get(url)
      ...
      .returnContent()
      .asString(StandardCharsets.UTF_8)
    val queryResponse: QueryResponse = Try(parse(response)) match {
      case Success(ok) =>
        new QueryResponse(
...
/**
 * @example http://ip:8081/jars/91ab0728-1156-4c65-a769-40ec143937dc_1.jar/run?entry-class=com.czl.Test&parallelism=1
 */
